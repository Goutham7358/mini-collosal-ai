...
So, the core problem. Modern language models have hundreds of millions to billions of parameters. A single GPU simply cannot hold all the model weights, gradients, and optimizer states needed for training. You need to split the work across multiple GPUs.

But there are multiple ways to do that --- Data Parallelism replicates the whole model and averages gradients, Tensor Parallelism splits individual layers, Pipeline Parallelism splits groups of layers across devices, and ZeRO shards the optimizer memory. Each one has a very different communication pattern. DP does a single large all-reduce at the end of a step. TP does two all-reduces inside every single transformer block. PP just sends small activation tensors between neighboring stages.

The question is: which strategy works best, and does it depend on the hardware you are running on? That is what we set out to study.

What makes our project different is that we did not use any high-level framework. We implemented all four strategies from scratch using only low-level \texttt{torch.distributed} calls like send, recv, and all-reduce. We then composed them into hybrid configurations using a unified plugin, tested on two different cluster setups, and ran experiments on two model architectures --- GPT-2 which is decoder-only, and T5 which is encoder-decoder.
...
Thanks Nadhiya. Let me walk through what we actually built.
...
Everything is implemented from scratch using just four primitives from \texttt{torch.distributed}.
...
When we add ZeRO-1, the gap gets even wider: Good 3,044 versus Bad 2,416, a 21\% difference. ZeRO adds extra optimizer-state communication to the DP axis, making it even more sensitive to link speed.
...
Over the course of this project, we built Mini Colossal-AI --- four parallelism strategies implemented entirely from scratch using low-level PyTorch distributed primitives. We then composed them through a unified 3D plugin that handles any combination of DP, PP, and TP.

We also implemented T5-base from scratch as our second model architecture, alongside three sizes of GPT-2.

...
\begin{frame}{Methodology: Unified 3D Hybrid Plugin}
  \begin{columns}[T]
  \begin{column}{0.55\textwidth}
    \includegraphics[width=\textwidth]{fig_ppt_3d_mesh_v3.png}
  \end{column}
  \begin{column}{0.42\textwidth}
    \small
...
% --- GPT-2 & T5 Hybrid Placement (Meena + Dinesh) ---
\begin{frame}{Results: Hybrid Placement --- GPT-2 vs T5}
  \small \textit{Setup: dp=2, pp=2, tp=2 on 8 GPUs across 2 nodes}

  \begin{columns}
  \begin{column}{0.48\textwidth}
    \centering
    \textbf{GPT-2 Medium (354M)}\\[0.3em]
    \includegraphics[width=\textwidth]{fig_ppt_gpt2_three_way.png}
  \end{column}
  \begin{column}{0.48\textwidth}
    \centering
    \textbf{T5-base (237M)}\\[0.3em]
  ...
  \small
  \begin{itemize}
    \item Both models: Good $\approx$ TP-inter $\gg$ Bad (DP inter-node). Bottleneck is \textbf{DP gradient sync} ($\sim$1.4 GB), not TP.
    \item GPT-2: Good is \textbf{+20\%} over Bad. With ZeRO-1: \textbf{+26\%} (3,044 vs 2,416).
    \item T5: Good is \textbf{+27\%} over Bad --- larger penalty because T5 has 60 vs 48 TP all-reduces (cross-attention) and is more \textbf{communication-bound}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  ...
  \begin{column}{0.55\textwidth}
    ...
    \begin{itemize}
      ...
      \item GPT-2 (Small / Medium / Large) implementation
